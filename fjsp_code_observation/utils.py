...
def op_is_ready(td: TensorDict):
    ...
    `is_ready[o] = True` <=>
    > операцию `o` можно планировать прямо сейчас,
    > если выбрать подходящую машину.
    Это чисто логическое состояние, не action mask.

    `ops_adj[..., 0]` — матрица предшественников
    ops_adj.shape = (bs, n_ops, n_ops, 2)
    * `[..., 0]` -> predecessor adjacency
    * `[..., 1]` -> successor adjacency

    `ops_adj[..., 0]` имеет форму (bs, n_ops, n_ops)
    Интерпретация:
    ops_adj[b, i, j, 0] = 1 <=> операция j — предшественник операции i

    `td["finish_times"][..., None]`
    ```
    finish_times.shape = (bs, n_ops)
    -> (bs, n_ops, 1)
    ```
    Это столбец времен окончания операций.

    `torch.bmm(pred, finish_times)` - делает матричное умножение, но сразу для батча матриц.
    ```
    torch.bmm(
        (bs, n_ops, n_ops),
        (bs, n_ops, 1)
    )
    -> (bs, n_ops, 1) - результат перемножения матриц по батчам
    ```

    Смысл: для каждой операции i берём время окончания её предшественника.
    Почему это работает?
    * В строке `i` матрицы `pred` стоит ровно одна единица (или ноль, если предшественника нет)
    * Умножение — это просто выбор нужного `finish_time`
    Пример:
    ```
    pred row for op2 = [0, 1, 0]
    finish_times      = [5, 12, inf]
    -> 0*5 + 1*12 + 0*inf = 12
    ```
    ...
    Broadcast -> сравнение с каждой операцией.
    Получаем:
    is_ready[b, o] = True <=> predecessor(o) закончил выполнение к текущему времени

    ### Что с первой операцией job?
    Для первой операции job:
    * строка `pred[i] = 0`
    * произведение = `0`

    0 <= time   → True
    > первая операция job готова сразу (если не запланирована)
    ...
    ```
    Разбор:
    ma_assignment.shape = (bs, num_mas, n_ops)
    sum(1) → (bs, n_ops)

    Если операция назначена **хотя бы на одну машину**:
    is_scheduled[o] = True

    Финальное условие готовности
    return torch.logical_and(is_ready, ~is_scheduled)

    Операция готова <=>
    | Условие         | Смысл                         |
    | --------------- | ----------------------------- |
    | `is_ready`      | предшественник завершён       |
    | `~is_scheduled` | операция ещё не запланирована |
    ...
    * job_in_process
    ...
    Это намеренно:
    `is_ready` = логическая готовность по графу,
    а реальная выполнимость проверяется позже через `action_mask`.
    """
...
def get_job_ops_mapping(
    ...
    # ХАК: последний job растягивают до n_ops_max
    #
    # Зачем?
    # В генераторе:
    # - реальные операции могут занимать только часть диапазона
    # - padding операции идут в конце
    #
    # Но чтобы:
    # - не было shape conflicts
    # - чтобы каждая операция имела какой-то job id
    #
    # они принудительно считают, что:
    # последний job включает все padding операции
    #
    # А потом padding будет удалён через pad_mask.
    # Это трюк для упрощения логики.
    # Важно: padding потом обнуляется в _decode_graph_structure.

    # here we will generate the operations-job mapping:
    # Therefore we first generate a sequence of operation ids and expand it the size of the mapping matrix:
    # (bs, jobs, max_ops)
    ops_seq_exp = torch.arange(n_ops_max, device=device)[None, None].expand(bs, num_jobs, -1)
    # Создание сетки op id
    # torch.arange(n_ops_max)[None, None] - добавляем 2 оси (1, 1, n_ops_max)
    # .expand(bs, num_jobs, -1) -> .shape = (bs, num_jobs, n_ops_max) (это копия в view, без реального копирования памяти)
    #
    # Пример для одного батча:
    # job0: [0,1,2,3,4,5]
    # job1: [0,1,2,3,4,5]
    # job2: [0,1,2,3,4,5]

    # (bs, jobs, max_ops)  # expanding start and end operation ids
    # start_op_per_job[..., None] / end_op_per_job[..., None] -> .shape = (bs, num_jobs, 1)
    # .expand_as(ops_seq_exp) -> .shape = (bs, num_jobs, max_ops)
    ...
    # (bs, jobs, max_ops)
    # Условие создаёт маску: True, если op_id ∈ [start_job, end_job]
    # nonzero возвращает индексы всех True в виде [batch_id, job_id, op_id] (shape = (N, 3))
    ...
    # ops_job_map[:, 1] - job_ids
    # Пример split:
    # x = torch.tensor([10, 11, 12, 13, 14, 15])
    # x.split(2)   # разбиение на чанки
    # [[10, 11],
    #  [12, 13],
    #  [14, 15]]
    # Пример stack:
    ...
    #  [4,5,6]]
    #
    # В итоге обработка может быть такой:
    # 1) tensor([
    #      [0, 0, 0],
    #      [0, 0, 1],
    #      [0, 1, 2],
    ...
    #      ...
    #    ])
    # 2) batch 0 chunk: [0,0,1,1,2,2]
    #    batch 1 chunk: [0,0,1,1,2,2]
    #    ...
    # 3) [
    #      [0,0,1,1,2,2],   # batch 0
    #      [0,0,1,1,2,2]    # batch 1
    ...
    ops_job_bin_map = torch.scatter_add(
        input=ops_job_map.new_zeros((bs, num_jobs, n_ops_max)),  # тензор из нулей с формой (bs, num_jobs, n_ops_max)
        dim=1,
        index=ops_job_map.unsqueeze(1),
        src=ops_job_map.new_ones((bs, num_jobs, n_ops_max)),   # тензор из единиц с формой (bs, num_jobs, n_ops_max)
    )
    # Как работает scatter_add?
    # output[ ..., index[...], ... ] += src[...]   по оси dim.
    # В нашем случае: dim=1 (ось job)
    # ops_job_bin_map[b, index[b, 0, op], op] += 1
