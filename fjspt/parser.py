...
def parse_job_line(line: tuple[int]) -> tuple[ProcessingData]:
    """
    Формат строки в файле обычно такой:
    [кол-во операций] [кол-во машин для оп.1] [машина1] [время1] [машина2] [время2]...

    Как она работает:
    1. Берет первое число — это сколько операций в этой работе.
    2. Идет циклом по этим операциям.
    3. Для каждой операции смотрит, на скольких станках она может быть выполнена (num_pairs).
    4. Извлекает пары (номер станка, длительность).
    Собирает всё это в список списков.
    """
...
def read(proc_locs: Path, max_ops=None):
    ...
    return td, num_jobs, num_machines, num_trucks, max_ops_per_job


def file2lines(loc: Path | str) -> list[list[int]]:
    """
    Эта функция выполняет предварительную очистку и парсинг текстового файла, превращая его из набора строк
    в структурированный список чисел (матрицу).
    Пример:
    10  5
    2  1  1  20.0
